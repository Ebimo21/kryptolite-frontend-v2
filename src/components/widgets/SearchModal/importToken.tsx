    ...
    <div className="flex flex-col">
      ...
      {tokens.map((token) => {
        ...
        return (
          <div key={token.address} className="grid gap-1">
            {list !== undefined ? <div className="bg-primary-400">via {list.name}</div> : <div>Unknown Source</div>}
            ...
            {chainId && (
              <div className="flex  items-center justify-between w-full">
                ...
                <Link to={getBscScanLink(token.address, "address", chainId)}>View on BscScan</Link>
              </div>
            )}
          </div>
        );
      ...
      <div className="flex justify-between items-center">
        <div className="flex items-center" onClick={() => setConfirmed(!confirmed)}>
          ...
          <p className="ml-2" style={{ userSelect: "none" }}>
            I understand
          </p>
        </div>
        <Button
