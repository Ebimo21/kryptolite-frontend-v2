...
const HarvestAction = ({ earnings, pid }: FarmCardActionsProps) => {
  ...
  return (
    <div
      className="flex md:justify-between mb-5 md:mb-0 border p-5 rounded-lg w-full
    ...
    >
      <div>
        ...
        <div className={`font-medium text-lg ${disabledClass}`}>
          {rawEarningsBalance.eq(0)
            ? rawEarningsBalance.toFixed(3)
            : rawEarningsBalance.toFixed(8)}
          {earningsBusd > 0 && (
